...
`__dirname`：获取<font color=red>当前执行文件所在**目录**</font>的完整的路径名

`__filename`：获取**当前执行文件的绝对路径**，带有文件名以及文件拓展名

`process.cwd()`：获取当前执行node命令时候的**文件夹目录名**
...
- **path.resolve([...paths])：**将路径片段**解析为一个文件的绝对路径**
...
API：[Nodejs官网：os模块](http://nodejs.cn/api/os.html)
...
- 在 `url.parse()方法中添加第二个参数，将query属性转换为对象`

![](./imgs/urlDemo_02.jpg)

- 更多URL模块的API参考 [Nodejs官网：URL模块](http://nodejs.cn/api/url.html)